<|
  "AtomicStateSystem" -> <|
    ...
    "tests" -> {
      (* Symbol Leak *)
      testSymbolLeak[GenerateMultihistory[AtomicStateSystem[a_ :> a + 1], 0, MaxEvents -> 5]],
      ...
      (* Parameters *)
      testUnevaluated[GenerateMultihistory[AtomicStateSystem[1 -> 2], 1, MaxGeneration -> -1],
                      {GenerateMultihistory::invalidParameter}],

      Function[{rules, parameters, init, expectedCreatedExpressions},
          VerificationTest[allExpressions @ GenerateMultihistory[AtomicStateSystem[rules], init, parameters],

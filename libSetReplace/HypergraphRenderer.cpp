...
static void ffmpeg_encoder_set_frame_yuv_from_rgb(uint8_t* rgb) {
  ...
  sws_context = sws_getCachedContext(
      sws_context, c->width, c->height, AV_PIX_FMT_RGB32, c->width, c->height, AV_PIX_FMT_YUV420P, 0, NULL, NULL, NULL);
...
void ffmpeg_encoder_glread_rgb(uint8_t** rgb, GLubyte** pixels, unsigned int width, unsigned int height) {
  ...
  *pixels = static_cast<GLubyte*>(realloc(*pixels, nvals * sizeof(GLubyte)));
  *rgb = static_cast<uint8_t*>(realloc(*rgb, nvals * sizeof(uint8_t)));
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      ...
      for (k = 0; k < format_nchannels; k++) (*rgb)[cur_rgb + k] = (*pixels)[cur_gl + k];
    }
  }
}
...
static int model_finished(void) { return nframes >= max_nframes; }
...
bool HypergraphRenderer::renderEvolutionVideo(const std::string& filename, const std::function<bool()>& shouldAbort) {
  return implementation_->renderEvolutionVideo(filename, shouldAbort);
}
}  // namespace SetReplace

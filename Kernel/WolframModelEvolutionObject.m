...
SetUsage @ "
LibSetReplaceSet[$$] is an expression attached to a low-level instance of the evolution object.
";
...
evolutionDataQ[3, data_Association] :=
  Keys[data] === {
    $version, $libSetReplaceSet, $rules, $maxCompleteGeneration, $terminationReason, $atomLists, $eventRuleIDs,
    $eventInputs, $eventOutputs, $eventGenerations};
...
migrateEvolutionObjectData[data_, 2, 3] :=
  Join[<|$version -> 3, $libSetReplaceSet -> Missing["Migrated"]|>, KeyDrop[data, $version]];

WolframModelEvolutionObject[data_] /;
    Lookup[data, $version, 1] < $currentVersion && evolutionDataQ[data] := WolframModelEvolutionObject[
  Fold[migrateEvolutionObjectData[#, #2[[1]], #2[[2]]] &,
       data,
       Partition[Range[Lookup[data, $version, 1], $currentVersion], 2, 1]]];

...
I have been thinking about an efficient and physically-reasonable solution to these issues for almost half-a-year now, and finally, I have a chance to explain it in this note.
...
And while it does solve the issues described above, it still has severe problems, in my opinion.
...
<img src="Images/RedundantGlobalMultiway.png" width="387">
...
Second, it is hard to distinguish between purely classical space evolving in disconnected regions in parallel (i.e., spacelike events), and multiway branching that happens due to ambiguities in rule application order (i.e., branchlike events).
...
In fact, generally speaking, there might not even be sufficient information left to distinguish between branchlike and spacelike pairs of events.
...
In that implementation, the datastructure is simply a set of edge structs (called `Expression`s there), and it simply has information about events that have created and destroyed it:
...
Furthermore, let's use a little helper function which would helpfully label the causal graph vertices with the input and output edges of the corresponding events:
...
However, the singleway system would terminate immediately after that, as there is only a single edge left now `{2, 3}`, `{1, 2}` has been deleted, and the second rule requires both `{1, 2}` and `{2, 3}` as inputs.
...
However, we need to discuss in more detail what it means exactly for the two events to be spacelike, branchlike, or timelike.
...
And if we had an event matching both `{v, f, 1}` and `{v, f, 2}` as inputs, it would simultaneously merge different pieces of spaces, and two different branches.
...
As soon as we hit an element from the `first` 's lightcone, we stop the recursion, and what kind of an element we have intersected.
...
A more interesting case would be to consider a system of rules where some are spacelike (i.e., only operate on pairwise spacelike sets of elements), and some are purely (or impurely) branchlike (i.e., they only operate on branchlike separated edges, but not on spacelike ones).
